...
// GET /api/nvr/status
// OWASP A05: absolute filesystem paths are internal deployment detail —
// only surfaced to ADMIN. Non-admin callers get just the running/health flag.
router.get('/status', authenticate, (req, res) => {
  const running = nvrRunning();
  const binExists = fs.existsSync(NVR_BIN);
  const base = { daemon_running: running, config_exists: fs.existsSync(NVR_CONF) };
...
router.post('/reload', authenticate, requireRole('ADMIN'), (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Config reload signal sent (SIGHUP)' });
  } catch (err) {
    console.error('[nvr:reload]', err);
    res.status(500).json({ error: 'Failed to signal daemon' });
  }
});

// GET /api/nvr/config — read current mnvr.conf
// ADMIN-only: the config file can contain camera/ONVIF credentials.
router.get('/config', authenticate, requireRole('ADMIN'), (req, res) => {
  try {
    if (!fs.existsSync(NVR_CONF)) return res.status(404).json({ error: 'Config file not found' });
...
router.post('/discover', authenticate, requireRole('ADMIN', 'OPERATOR'), async (req, res) => {
  try {
    ...
    console.error('[nvr:discover]', err);
